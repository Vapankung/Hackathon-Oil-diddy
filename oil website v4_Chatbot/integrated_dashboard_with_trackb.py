...
trackb_policy_bundle = run_trackb_policy_suite(data_roots)
trackb_policy_files = trackb_policy_bundle.get("files", {})
trackb_policy_panel = trackb_policy_bundle.get("worldbank_panel", pd.DataFrame())
trackb_policy_causal = trackb_policy_bundle.get("causal", {})
trackb_thai_context = trackb_policy_bundle.get("thai_context", pd.DataFrame())
...
with st.sidebar:
    ...
    st.divider()
    st.markdown("### Track B policy simulation")
    price_shock_input = st.slider("Global fuel price shock (%)", -10.0, 40.0, 12.0, 0.5)
    subsidy_input = st.slider("Policy fuel subsidy (%)", 0.0, 30.0, 8.0, 0.5)
    cash_transfer_input = st.slider("Cash transfer per household (THB/month)", 0.0, 1000.0, 250.0, 25.0)
    budget_input = st.slider("Optimization budget (billion THB)", 5.0, 120.0, 40.0, 1.0)
...
# -----------------------------
# Track B policy engines
# -----------------------------
trackb_abm_result = agent_based_money_resilience(
    price_shock_pct=float(price_shock_input),
    subsidy_pct=float(subsidy_input),
    cash_transfer_thb=float(cash_transfer_input),
)
trackb_opt_result = optimize_policy_mix(
    price_shock_pct=float(price_shock_input),
    budget_billion_thb=float(budget_input),
)
...
# -----------------------------
with main_tabs[7]:
    st.markdown('<p class="section-label">H · Track B policy modeling core (causal inference + agent-based modeling + optimization)</p>', unsafe_allow_html=True)
    st.markdown(
        f'<div class="card">{_kind_badge("measured")}{_kind_badge("estimated")}<h3>Scope lock: only Track B datasets</h3>'
        '<p>This section is constrained to World Bank Global Fuel Prices, World Bank Subsidies & Price Control, OWID, DOEB Open Data, and NESDC socio-economic files. It reuses the same upload/auto-discovery pipeline as the rest of this dashboard.</p></div>',
        unsafe_allow_html=True,
    )

    c1, c2, c3 = st.columns(3)
    with c1:
        _value_box("WB monthly price panel rows", str(int(len(trackb_policy_panel))) if not trackb_policy_panel.empty else "0")
    with c2:
        _value_box("Countries in panel", str(int(trackb_policy_panel["country"].nunique())) if (not trackb_policy_panel.empty and "country" in trackb_policy_panel.columns) else "0")
    with c3:
        _value_box("Policy file detected", "Yes" if "wb_policy" in trackb_policy_files else "No")

    st.markdown("**Detected Track B policy files**")
    if trackb_policy_files:
        st.dataframe(pd.DataFrame(sorted(trackb_policy_files.items()), columns=["role", "path"]), use_container_width=True, hide_index=True)
    else:
        st.warning("No Track B policy files found yet. Upload the Track_B_Adaptive_Infrastructures_Datasets zip/folders.")

    st.markdown("### 1) Causal inference (TWFE)")
    if not trackb_policy_causal or not trackb_policy_causal.get("ready"):
        st.info((trackb_policy_causal or {}).get("message", "Causal model is not ready; check whether WB prices + policy files were parsed."))
    else:
        st.dataframe(pd.DataFrame([trackb_policy_causal]), use_container_width=True, hide_index=True)

    st.markdown("### 2) Agent-based modeling")
    st.caption(f"Scenario input → shock={price_shock_input:.1f}% | subsidy={subsidy_input:.1f}% | transfer={cash_transfer_input:.0f} THB/month")
    if trackb_abm_result.get("ready"):
        top1, top2 = st.columns(2)
        with top1:
            _value_box("Weighted stress change (pp)", f"{trackb_abm_result['weighted_stress_delta_pp']:.2f}")
        with top2:
            _value_box("Affordability breach (%)", f"{trackb_abm_result['affordability_breach_rate_pct']:.2f}")
        st.dataframe(pd.DataFrame(trackb_abm_result.get("group_results", [])), use_container_width=True, hide_index=True)

    st.markdown("### 3) Optimization")
    st.caption(f"Budget constraint = {budget_input:.0f} billion THB")
    if trackb_opt_result.get("ready"):
        best = trackb_opt_result.get("best_policy", {})
        b1, b2, b3 = st.columns(3)
        with b1:
            _value_box("Best subsidy (%)", f"{best.get('subsidy_pct', np.nan):.1f}")
        with b2:
            _value_box("Best transfer (THB)", f"{best.get('cash_transfer_thb', np.nan):.0f}")
        with b3:
            _value_box("Estimated budget (B THB)", f"{best.get('estimated_budget_billion_thb', np.nan):.2f}")
        st.dataframe(pd.DataFrame([best]), use_container_width=True, hide_index=True)
    else:
        st.info(trackb_opt_result.get("message", "Optimization is not ready."))

    st.markdown("### 4) Thailand public-data context (OWID + DOEB + NESDC)")
    if trackb_thai_context.empty:
